...
<script>
  ...
  export default {
    ...
    created() {
      if (this.isEdit) {
        getProductCate(this.$route.query.id).then(response => {//得到产品种类
          this.productCate = response.data;
        });

        getProductAttrInfo(this.$route.query.id).then(response => {//得到产品属性

          if (response.data != null && response.data.length > 0) {
            this.filterProductAttrList = [];

            for (let i = 0; i < response.data.length; i++) {
              console.log(response.data[i].attributeCategoryId);
